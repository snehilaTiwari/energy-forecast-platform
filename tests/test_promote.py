...
def _mock_version(version: str, run_id: str) -> MagicMock:
    v = MagicMock()
    v.version = version
    v.run_id = run_id
    return v
...
def test_get_production_rmse_returns_rmse_from_run():
    client = MagicMock()
    client.get_latest_versions.return_value = [_mock_version("2", "run-abc")]
...
def test_get_best_challenger_rmse_and_version_picks_lowest_rmse():
    client = MagicMock()
    client.get_latest_versions.return_value = [
        _mock_version("3", "run-1"),
        _mock_version("4", "run-2"),
    ]
